...
public class EmfService {
    ...
    public Resource loadResource(String filePath) throws Exception {
        ...
        try {
            ...
            options.put(org.eclipse.emf.ecore.xmi.XMLResource.OPTION_DEFER_IDREF_RESOLUTION, true);
            resource.load(options);
            
            // Register all packages found in the resource (without overwriting existing ones)
            for (EObject root : resource.getContents()) {
                if (root instanceof EPackage) {
                    ...
                    Object existing = EPackage.Registry.INSTANCE.get(nsURI);
                    if (!(existing instanceof EPackage)) {
                        // Register globally and in this ResourceSet's registry
    ...
    /**
     * Load an XMI (or any registered) model resource at modelPath, preserving the
     * file extension. If the file does not exist, a new resource is created and saved.
     */
    public Resource loadOrCreateModelResource(String modelPath) throws Exception {
        ...
        if (resource == null) {
            resource = resourceSet.createResource(uri);
            if (resource == null) {
                throw new Exception("Failed to create resource for: " + modelPath);
            }
        }
        try {
            if (file.exists()) {
                // Load with ID handling options
                Map<String, Object> options = new HashMap<>();
                options.put("USE_ENCODED_ATTRIBUTE_STYLE", Boolean.TRUE);
                options.put("HREF_HANDLER", Boolean.TRUE);
                resource.load(options);
            } else {
                // ensure parent directories
                File parent = file.getParentFile();
                if (parent != null) parent.mkdirs();
                // Save with ID options
                Map<String, Object> options = new HashMap<>();
                options.put("USE_ENCODED_ATTRIBUTE_STYLE", Boolean.TRUE);
                resource.save(options);
            }
        } catch (Exception e) {
            throw new Exception("Failed to load/save model resource: " + e.getMessage());
        }
        return resource;
    }
